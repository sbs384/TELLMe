...
class LogME(object):
    def __init__(self, args=None, regression=False):
        """
    ...
    def score(self, features, src_embeddings, candidate_embeddings):
        ...
        candidate_num = src_embeddings.shape[1]
        dot_res = np.zeros((src_num, candidate_num))
        src_embeddings /= np.linalg.norm(src_embeddings, axis=2, keepdims=True)
        candidate_embeddings /= np.linalg.norm(candidate_embeddings, axis=2, keepdims=True)
        for i in range(src_num):
            for j in range(candidate_num):
                dot_res[i][j] = np.dot(src_embeddings[i][j], candidate_embeddings[i][j])
        
        print(f"dot_res_0: {dot_res[0][0]}")
        dot_res = (dot_res + 1) / 2
        labels = ((dot_res) / (1e-8 + np.sum(dot_res, axis=1, keepdims=True)))
        labels = np.concatenate(labels, 0)
        labels = np.array(labels)
        print(labels)
        '''
    
        score = self.logme(features, labels)
        # import pdb; pdb.set_trace()
        end_time = time.time()
